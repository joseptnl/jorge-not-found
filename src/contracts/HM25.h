...
constexpr uint64 HM25_MAX_OWNERS = 16;
constexpr uint64 HM25_MAX_COOWNER = 8;
constexpr uint64 HM25_INITIAL_MAX_VAULTS = 131072ULL; // 2^17
constexpr uint64 HM25_MAX_VAULTS = HM25_INITIAL_MAX_VAULTS * X_MULTIPLIER;
// HM25 asset name : 23727827095802701, using assetNameFromString("HM25") utility in test_util.h
static constexpr uint64 HM25_ASSET_NAME = 23727827095802701;

constexpr uint64 HM25_REGISTERING_FEE = 5000000ULL;
constexpr uint64 HM25_RELEASE_FEE = 100000ULL;
constexpr uint64 HM25_RELEASE_RESET_FEE = 1000000ULL;
constexpr uint64 HM25_HOLDING_FEE = 500000ULL;
constexpr uint64 HM25_BURN_FEE = 0ULL; // Integer percentage from 1 -> 100
// [TODO]: Turn this assert ON when HM25_BURN_FEE > 0
//static_assert(HM25_BURN_FEE > 0, "SC requires burning qu to operate, the burn fee must be higher than 0!");

static constexpr uint64 HM25_MAX_FEE_VOTES = 64;
...
public:
    ...
    {
        id vaultName;
        Array<id, HM25_MAX_OWNERS> owners;
        Array<uint64, HM25_MAX_OWNERS> releaseAmounts;
        Array<id, HM25_MAX_OWNERS> releaseDestinations;
        uint64 balance;
    ...
    struct registerVault_locals
    {
        ...
        Array<id, HM25_MAX_OWNERS> tempOwners;
    ...
    struct voteFeeChange_locals
    {
        ...
        id currentAddr;
        uint64 realScore;
        HM25FeeVote currentVote;
        HM25FeeVote uniqueVote;
    ...
    struct getVaults_output
    {
        ...
        Array<uint64, HM25_MAX_COOWNER> vaultIds;
        Array<id, HM25_MAX_COOWNER> vaultNames;
    };
    ...
    {
        uint64 status;
        Array<uint64, HM25_MAX_OWNERS> amounts;
        Array<id, HM25_MAX_OWNERS> destinations;
    };
...
protected:
    // Contract states
    Array<Vault, HM25_MAX_VAULTS> vaults;
    ...
    Array<HM25FeeVote, HM25_MAX_FEE_VOTES> feeVotes;
    Array<id, HM25_MAX_FEE_VOTES> feeVotesOwner;
    Array<uint64, HM25_MAX_FEE_VOTES> feeVotesScore;
    uint64 feeVotesAddrCount;

    Array<HM25FeeVote, HM25_MAX_FEE_VOTES> uniqueFeeVotes;
    Array<uint64, HM25_MAX_FEE_VOTES> uniqueFeeVotesRanking;
    ...
    PRIVATE_FUNCTION_WITH_LOCALS(isValidVaultId)
        if (input.vaultId < HM25_MAX_VAULTS)
        {
            output.result = true;
        }
        else
    ...
    PRIVATE_FUNCTION_WITH_LOCALS(resetReleaseRequests)
        for (locals.i = 0; locals.i < HM25_MAX_OWNERS; locals.i++)
        {
    ...
    PUBLIC_PROCEDURE_WITH_LOCALS(registerVault)
        ...
        if (qpi.invocationReward() < HM25_REGISTERING_FEE)
        {
        ...
        locals.ownerCount = 0;
        for (locals.i = 0; locals.i < HM25_MAX_OWNERS; locals.i = locals.i + 1)
        {
        ...
        if (locals.ownerCount > HM25_MAX_OWNERS)
        {
            qpi.transfer(qpi.invocator(), qpi.invocationReward());
            return;
        }

        for (locals.i = locals.ownerCount; locals.i < HM25_MAX_OWNERS; locals.i = locals.i + 1)
        {
        ...
        // Find empty slot
        locals.slotIndex = -1;
        for (locals.ii = 0; locals.ii < HM25_MAX_VAULTS; locals.ii++)
        {
        ...
        for (locals.i = 0; locals.i < locals.ownerCount; locals.i++)
        {
            ...
            locals.count = 0;
            for (locals.j = 0; locals.j < HM25_MAX_VAULTS; locals.j++)
            {
                ...
                if (locals.tempVault.isActive)
                {
                    for (locals.k = 0; locals.k < (uint64)locals.tempVault.numberOfOwners; locals.k++)
                    {
                        ...
                        {
                            locals.count++;
                            if (locals.count >= HM25_MAX_COOWNER)
                            {
                                qpi.transfer(qpi.invocator(), qpi.invocationReward());
                                return;
                            }
        ...
        if (qpi.invocationReward() > HM25_REGISTERING_FEE)
        {
            qpi.transfer(qpi.invocator(), qpi.invocationReward() - HM25_REGISTERING_FEE);
        }
        ...
        state.totalRevenue += HM25_REGISTERING_FEE;
    _
    ...
    PUBLIC_PROCEDURE_WITH_LOCALS(releaseTo)
        ...
        //}
        if (qpi.invocationReward() > HM25_RELEASE_FEE)
        {
            qpi.transfer(qpi.invocator(), qpi.invocationReward() - HM25_RELEASE_FEE);
        }
        // [TODO]: Change this to
        //state.totalRevenue += state.liveReleaseFee;
        state.totalRevenue += HM25_RELEASE_FEE;
    ...
    PUBLIC_PROCEDURE_WITH_LOCALS(resetRelease)
        ...
        //}
        if (qpi.invocationReward() > HM25_RELEASE_RESET_FEE)
        {
            qpi.transfer(qpi.invocator(), qpi.invocationReward() - HM25_RELEASE_RESET_FEE);
        }
        // [TODO]: Change this to
        //state.totalRevenue += state.liveReleaseResetFee;
        state.totalRevenue += HM25_RELEASE_RESET_FEE;
    ...
    //
    //    qpi.transfer(qpi.invocator(), qpi.invocationReward());
    //    locals.nShare = qpi.numberOfPossessedShares(HM25_ASSET_NAME, id::zero(), qpi.invocator(), qpi.invocator(), HM25_CONTRACT_INDEX, HM25_CONTRACT_INDEX);
    //
    ...
    //    // [TODO]: Turn this ON when HM25_BURN_FEE > 0
    ...
    //        locals.realScore = qpi.numberOfPossessedShares(HM25_ASSET_NAME, id::zero(), locals.currentAddr, locals.currentAddr, HM25_CONTRACT_INDEX, HM25_CONTRACT_INDEX);
    ...
    PUBLIC_FUNCTION_WITH_LOCALS(getVaults)
        ...
        locals.count = 0ULL;
        for (locals.i = 0ULL; locals.i < HM25_MAX_VAULTS; locals.i++)
        {
    ...
    //    if (qpi.numberOfPossessedShares(HM25_ASSET_NAME, id::zero(), input.candidate, input.candidate, HM25_CONTRACT_INDEX, HM25_CONTRACT_INDEX) > 0)
    //    {
    //        output.result = 1ULL;
    //    }
    //    else
    ...
    INITIALIZE
        ...
        state.liveBurnFee = HM25_BURN_FEE;
        state.liveRegisteringFee = HM25_REGISTERING_FEE;
        state.liveReleaseFee = HM25_RELEASE_FEE;
        state.liveReleaseResetFee = HM25_RELEASE_RESET_FEE;
        state.liveHoldingFee = HM25_HOLDING_FEE;
    ...
    END_EPOCH_WITH_LOCALS
        for (locals.i = 0ULL; locals.i < HM25_MAX_VAULTS; locals.i++)
        {
            ...
            if (locals.v.isActive)
            {
                ...
                //}
                if (locals.v.balance >= HM25_HOLDING_FEE)
                {
                    locals.v.balance -= HM25_HOLDING_FEE;
                    state.totalRevenue += HM25_HOLDING_FEE;
                    state.vaults.set(locals.i, locals.v);
                }
                else
                {
                    ...
                    for (locals.j = 0; locals.j < HM25_MAX_OWNERS; locals.j++)
                    {
